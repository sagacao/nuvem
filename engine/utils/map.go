...
func (m *Map) UnsafeHas(key interface{}) bool {
	if m.m == nil {
		return false
	} else {
		if _, ok := m.m[key]; ok {
...
func (m *Map) IgnoreGetOne(ignore interface{}) interface{} {
	m.Lock()
	defer m.Unlock()
...
func (m *Map) UnsafeRangeTerm(f func(interface{}, interface{}) bool) {
	if m.m == nil {
		return
	}
	for k, v := range m.m {
...
func (m *Map) RLockRangeTerm(f func(interface{}, interface{}) bool) {
	m.RLock()
	defer m.RUnlock()
	m.UnsafeRangeTerm(f)
}
